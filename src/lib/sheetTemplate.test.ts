...
describe('applyTemplateToSheet', () => {
  ...
  it('매핑된 셀 값으로 설비 필드를 채운다', () => {
    const t = template({ cells: { 설비명: 'A6', 분류: 'C6', 제조사: 'B10' } });
    const result = applyTemplateToSheet(sheet, t);
    expect(result.name).toBe('공조기 4호기');
    expect(result.category).toBe('전기');
    expect(result.extraFields.제조사).toBe('삼성전자');
  });
  ...
  it('커스텀 필드는 상세사양으로 들어간다', () => {
    ...
    const result = applyTemplateToSheet(sheet, t);
    expect(result.상세사양).toEqual({ 가격: '1500000' });
  });
  ...
  it('라벨이 비었거나 값이 없는 커스텀 필드는 상세사양에서 빠진다', () => {
    ...
    const result = applyTemplateToSheet(sheet, t);
    expect(result.상세사양).toEqual({});
  });

  it('매핑 안 된 필드는 extraFields에 아예 없다(undefined로 덮어쓰지 않음)', () => {
    const t = template({ cells: { 설비명: 'A6' } });
    const result = applyTemplateToSheet(sheet, t);
    expect('제조사' in result.extraFields).toBe(false);
    expect('설치일' in result.extraFields).toBe(false);
  });
});
